...
namespace QYMSERVER.Tests
{
    public abstract class QYMSERVERTestBase : AbpIntegratedTestBase<QYMSERVERTestModule>
    {
        ...
        protected void UsingDbContext(Action<QYMSERVERDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<QYMSERVERDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<QYMSERVERDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<QYMSERVERDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<QYMSERVERDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<QYMSERVERDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<QYMSERVERDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<QYMSERVERDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<QYMSERVERDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<QYMSERVERDbContext, Task<T>> func)
        {
            T result;
